...
function Surface() {
  ...
  const { geometry, positions } = useMemo(() => {
    ...
    return { geometry: geo, positions: arr };
  }, []);
  ...
  return (
    <mesh ref={meshRef} geometry={geometry} rotation={[-Math.PI / 3, 0, Math.PI / 6]}>
...
function Axes() {
  ...
  return (
    <group rotation={[-Math.PI / 3, 0, Math.PI / 6]}>
      ...
      <Text position={[4, 0, 0]} fontSize={0.22} color={accentColor} font="/fonts/IBMPlexMono-Regular.ttf" anchorX="left">
        Strike Price
      </Text>
      <Text position={[0, 4, 0]} fontSize={0.22} color={accentColor} font="/fonts/IBMPlexMono-Regular.ttf" anchorX="center">
        Time to Maturity
      </Text>
      <Text position={[0, 0, 2.8]} fontSize={0.22} color={accentColor} font="/fonts/IBMPlexMono-Regular.ttf" anchorX="center">
        Implied Vol (σ)
      </Text>
    </group>
  );
